...
if ($action === 'delete' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt->execute([$id]);
    set_flash('success', 'Agendamento excluído com sucesso.');
...
<section class="card" style="margin-top:12px">
    <div style="display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap">
        <div>
            ...
            <p class="muted" style="margin:6px 0 0">Faça agendamentos para manter organização e controle dos clientes.</p>
        </div>
        <div class="actions">
            ...
            <form method="post" action="appointments.php?action=edit&id=<?php echo (int)$appointment['id']; ?>" class="grid grid--2">
                ...
                <div>
                    ...
                    <select id="status" name="status" required>
                        ...
                        <option value="concluido" <?php echo ($appointment['status'] === 'concluido') ? 'selected' : ''; ?>>Concluído</option>
        ...
        <?php elseif ($action === 'delete'): ?>
            <div>
                ...
                <div class="card" style="margin-top:12px">
                    ...
                    <div><b>Serviço:</b> <?php echo h($appointment['service_name'] ?? ''); ?></div>
                    <div><b>Horário:</b> <?php echo h((string)$appointment['scheduled_at']); ?></div>
                    <div style="margin-top:6px">
                        <span class="badge badge--<?php echo h($appointment['status'] ?? 'agendado'); ?>">
                            <?php echo h(status_label($appointment['status'] ?? 'agendado')); ?>
                        </span>
                    </div>
                </div>
                        ...
                        <tr>
                            ...
                            <td>
                                <span class="badge badge--<?php echo h($row['status']); ?>">
                                    <?php echo h(status_label($row['status'])); ?>
                                </span>
                            </td>
                            <td>
                                <div class="actions">
