...
function validate_client_input(string $name, string $phone): ?string
{
    if ($name === '') {
...
if ($action === 'create' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $error = validate_client_input($name, $phone);
    if ($error !== null) {
        ...
        <div style="margin-top:16px">
            <form method="post" action="clients.php?action=create" class="grid grid--2">
                ...
                <div>
                    ...
                    <input id="name" name="name" type="text" required pattern="[A-Za-zÀ-ÿ\s]+" title="Use apenas letras" value="">
                </div>
                <div>
                    <label for="phone">Telefone</label>
                    <input id="phone" name="phone" type="text" pattern="[0-9]*" title="Use apenas números" value="">
                </div>
    ...
    <?php elseif ($action === 'edit'): ?>
        <div style="margin-top:16px">
            <form method="post" action="clients.php?action=edit&id=<?php echo (int)$id; ?>" class="grid grid--2">
                <div>
                    <label for="name">Nome *</label>
                    <input id="name" name="name" type="text" required pattern="[A-Za-zÀ-ÿ\s]+" title="Use apenas letras" value="<?php echo h($client['name'] ?? ''); ?>">
                </div>
                <div>
                    <label for="phone">Telefone</label>
                    <input id="phone" name="phone" type="text" pattern="[0-9]*" title="Use apenas números" value="<?php echo h($client['phone'] ?? ''); ?>">
                </div>
                <div style="grid-column:1 / -1">
                    <label for="email">Email</label>
